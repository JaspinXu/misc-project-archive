function res=yearpower(crd,crd_cnt,mr_hgt,mr_wdt,ins_hgt,DNI,vector_light,beta)
    ...
    for i=1:crd_cnt
       for j=1:12
          for k=1:5
              res(i,j,k)=DNI(j,k)*nat(i,j,k)*0.75*nsb(i,j,k)*ncos(i,j,k)*mr_hgt*mr_wdt;
              if sin(crd(i,2))>0
