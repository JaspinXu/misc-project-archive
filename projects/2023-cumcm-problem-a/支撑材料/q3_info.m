...
beta=best_x(3);
  ...
  x1=best_m(1);x2=best_m(2);x3=best_m(3);
  load DNI.mat
...
% ncosa，ntra，nsba分别为四个效率根据12月求得，n也同样为12个月的,total为年平均热功率
    ...
    hot=zeros(12,1);
    for i=1:12
    hot(i)=sum(res(:,i).*mr_hgt(1:crd_cnt).^2)/sum(mr_hgt(1:crd_cnt).^2);
    end
    % hot(i)为i个月的单位面积功率
    tower_axis=dev_cu;% 塔的坐标
    crd_cnt; % 定日镜总面数
    t_s=sum(mr_hgt(1:crd_cnt).^2); % 定日镜总面积
    l=8-x2.*log(crd(:,1))-x3.*exp(-crd_md(:)); % 定日镜长度和宽度
    xy=zeros(crd_cnt,2);
for i=1:crd_cnt
   xy(i,1)=crd(i,1)*cos(crd(i,2));
   xy(i,2)=crd(i,1)*sin(crd(i,2))+dev_cu;
end% 定日镜xy坐标
ins_hgt=5+x1.*(1-exp(-crd(:,1)./100));% 定日镜z坐标
