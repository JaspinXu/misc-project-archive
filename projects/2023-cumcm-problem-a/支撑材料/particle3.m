%% 粒子群算法函数
function [gbest,res]=particle3(x_lb,x_ub,crd,crd_cnt,crd_md,beta,dr,max_layers,DNI,vector_light)
...
w = 0.9;  % 惯性权重
K = 7;  % 迭代的次数
vmax = [0.03,0.006,0.03]; % 粒子的最大速度
% 初始化粒子的位置和速度
x = zeros(n,narvs);
r_max=(max_layers-1)*dr;
for i=1:n
   x(i,1)=x_lb(1)+(x_ub(1)-x_lb(1))*rand(1);
   x(i,2)=x_lb(2)+(x_ub(2)-x_lb(2))*rand(1);
   x(i,3)=x_lb(3)+(x_ub(3)-x_lb(3))*rand(1);
   while ~(((8-x(i,2)*log(100))<(dr-5))&&((8-x(i,2)*log(r_max)-x(i,3))>=(2))&&((5+x(i,1)*(1-(exp(-(r_max))+1)/2))>(8-x(i,2)*(log(r_max)+log(100))/2)))
       x(i,1)=x_lb(1)+(x_ub(1)-x_lb(1))*rand(1);
       x(i,2)=x_lb(2)+(x_ub(2)-x_lb(2))*rand(1);
       x(i,3)=x_lb(3)+(x_ub(3)-x_lb(3))*rand(1);
   end
...
penalty=100000;
objfun=@(x)target_fun3(x,crd_md,crd,crd_cnt)+penalty*abs(yearpower3(crd,crd_cnt,crd_md,x(1),x(2),x(3),DNI,vector_light,beta)-60);
% 计算适应度
...
res=target_fun3(gbest,crd_md,crd,crd_cnt);
